...
if __name__ == '__main__':
    ...
    # Cplex求解时需要用到的变量
    user_size, res_size, server_size, delta_links = 100, 3, 10, 5
    bids, caps, S = {}, {}, {}
    delta = {}
    R_rate = 1
    ...
    unit_price = [43.5, 15.2, 0.35]  # 元/每月
    ...
    while cpuf:
        cpuf = False
        r_cpu = np.random.normal(loc=92, scale=1.0, size=server_size)
    ...
    while ramf:
        ramf = False
        r_ram = np.random.normal(loc=160, scale=2.0, size=server_size)
    ...
    while hdf:
        hdf = False
        r_harware = np.random.normal(loc=1536, scale=4.0, size=server_size)
    ...
    np.savetxt("data_bi.txt", bi, fmt="%.2f")
    np.savetxt("data_si.txt", si, fmt="%d")
    np.savetxt("data_delta.txt", _delta_, fmt="%d")
    np.savetxt("data_cr.txt", c_, fmt="%d")
